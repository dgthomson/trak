...
$(function() {
      $('table tbody tr').mouseover(function() {
		 ...
		 if(tagName != 'INPUT' && !jQuery(event.target).hasClass('title')){
			...
			var newWindow = window.open('simple_document.aspx?doc='+ $('input:first', this).val(),'open_window','menubar, toolbar, location, directories, status, scrollbars, resizable, dependent, width=640, height=480, left=0, top=0');
			<% }else{ %>
			var newWindow = window.open('split_document.aspx?doc='+ $('input:first', this).val(),'open_window','menubar, toolbar, location, directories, status, scrollbars, resizable, dependent, width=640, height=480, left=0, top=0');	
			<% } %>
      	 }else{
			if(tagName != 'INPUT')
