...
// This section could be modified to give descriptive entries in calendar
// 	if ($_nameCache[$this->data[$this->config->text[5]["name"]]] != "") {
// 		$str.= sprintf("<text><![CDATA[CACHE:%s]]></text>", $_nameCache[$this->data[$this->config->text[5]["name"]]] );
// 	}
// 	else
// 	{
// 		$dbQuery = mysql_query("SELECT name FROM trak.mau_patient WHERE id = '". $this->data[$this->config->text[5]["name"]] ."' LIMIT 1;");
// 		if (!$dbQuery) {
//    		echo 'Could not run query: ' . mysql_error();
//    		exit;
// 	};
// 		$row = mysql_fetch_array($dbQuery, MYSQL_ASSOC);
// 		$str.= sprintf("<text><![CDATA[LOOKUP:%s]]></text>", $row['name'] );
// 		$_nameCache[$this->data[$this->config->text[5]["name"]]] = $row['name'];
// 	};
// 5 //		global $_nameCache;
